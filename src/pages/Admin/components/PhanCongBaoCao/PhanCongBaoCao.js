...
import { deletePhanCong, getAllPhanCong, getKdclData, getPhongBan, savePhanCong, updatePhanCong } from "../../../../services/apiServices";
import { useLocation } from "react-router-dom";
import { Modal, Button, Form } from 'react-bootstrap';
import { Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
const PopupForm = ({ show, handleClose, idChuanKiemDinh, phongBan, soLuongTieuChuan , fetchData, formData, setFormData}) => {
...
const PhanCongBaoCao = () => {
  ...
  const xoaPhanCong = async (idPhanCong) => {
    const response = await deletePhanCong(idPhanCong);
    if(response === "OK"){
      fetchPhanCong();
    }
  }
  ...
  return (
    <div className="content" style={{ background: "white", margin: "20px" }}>
      <PopupForm show={show} handleClose={handleClose} idChuanKiemDinh={ChuanKiemDinh_ID} phongBan={phongBan} soLuongTieuChuan={filterChuanKiemDinh[0].soLuongTieuChuan} fetchData={fetchPhanCong} formData={formData} setFormData={setFormData}/>
        ...
        <Table className="font-Inter">
          <TableHead>
            <TableRow id="table-row-color">
              ...
              <TableCell className="text-white">Ban</TableCell>
              <TableCell className="text-white">Tiêu chuẩn</TableCell>
              <TableCell className="text-white text-center" ><button className="btn btn-success" onClick={handleShow}>Thiết lập nhóm</button></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {phanCong.map((item, index) => (
              <TableRow key={item.id}>
                ...
                <TableCell>Tiêu chuẩn : <TieuChuan sttBatDau={item.sttTieuChuanBatDau} sttKetThuc={item.sttTieuChuanKetThuc}/>
                </TableCell>
                <TableCell className="text-center">
                 <button className="btn btn-primary mb-2" onClick={() => suaPhanCong(item.idPhanCong, item.phongBan.idPhongBan, item.sttTieuChuanBatDau, item.sttTieuChuanKetThuc)}>Chỉnh sửa</button><br/>
                 <button className="btn btn-danger" onClick={() => xoaPhanCong(item.idPhanCong)}>Xóa</button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
